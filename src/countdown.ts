export type Unit = 'ms' | 'sec' | 'min' | 'hour'
...
export type AddCountdown = (
  taskName: string,
  taskParams: TaskParams,
  task: (remaining: number) => void
) => void

export type ClearCountdown = (taskName: string | Array<string>) => void

function processTaskRemaining(
  { remaining, unit }: TaskParams,
  ticktockStep: number
) {
